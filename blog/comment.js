...
async function loadComments(postId) {
    try {
        const response = await fetch(`/blog/comment_api.php?action=list&post_id=${postId}`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (submitBtn) {
        submitBtn.addEventListener('click', async function() {
            ...
            try {
                const response = await fetch('/blog/comment_api.php?action=create', {
                    method: 'POST',
...
async function handleReact(e) {
    ...
    try {
        const response = await fetch('/blog/comment_api.php?action=react', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
